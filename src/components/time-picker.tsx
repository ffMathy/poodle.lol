    ...
    return <>
        ...
        <div class="mb-5">
            {times.value.map((time, timeIndex) => 
                <div 
                    class="flex mb-2" 
                    key={`time-${props.day.toISOString()}-${time.toISOString()}-${timeIndex}`}
                >
                    <TimePicker
                        key={`time-picker-${props.day.toISOString()}-${time.toISOString()}-${timeIndex}`}
                        selectedTime={time}
                        onChange$={newTime => {
                    ...
                    <button 
                        ...
                        onClick$={() => {
                            const timesCopy = times.value.filter((_, index) => index !== timeIndex);
